...
- 색인 신청은 대표 도메인 `https://sosotime.com` 주소만 사용합니다.
- `http://`, `www`, `localhost`, `127.0.0.1`, `*.pages.dev` 주소는 제출하지 않습니다.
- `?category=`, `?rank=`, `?q=` 같은 필터/검색 파라미터가 붙은 URL은 중복 색인 가능성이 있어 제출하지 않습니다.
- 먼저 사이트맵을 제출하고, 그다음 메인 페이지와 최신/중요 게시글만 URL 검사로 색인 요청합니다.
...
2. 소유 확인 후 사이트맵 제출: `https://sosotime.com/sitemap.xml`
3. robots 확인: `https://sosotime.com/robots.txt`
4. 웹 페이지 수집 요청에는 메인 페이지와 최신 게시글 위주로 제출

## Bing Webmaster Tools 제출 순서

1. 사이트 추가: `https://sosotime.com`
2. 사이트맵 제출: `https://sosotime.com/sitemap.xml`
3. URL 제출에는 메인 페이지와 최신 게시글 위주로 제출
